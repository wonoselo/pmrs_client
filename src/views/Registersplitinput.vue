        ...
        >
          ...
          >
            ...
            <DxForm>
              ...
              <DxItem data-field="Kategori" :validation-rules="validationRules.position"/>
              <DxItem data-field="Sitetype" :visible="isVis" :validation-rules="validationRules.position"/>
              <DxItem data-field="Landcover" :visible="isVis" :validation-rules="validationRules.position"/>
              <DxItem data-field="Replant" :visible="isVis2" :validation-rules="validationRules.position"/>
              <DxItem data-field="Jenisbelukar" :visible="isVis" :validation-rules="validationRules.position"/>
            </DxForm>
          </DxEditing>
          <!-- End Create Setting-->
...
const sectors = ['SAL II','PNB','SSU VI','MLJ','HBL','BPU','TRH','SAL I','SWP','WKL','ISM','BPE','SDC','LGM','DLS','CKM','PTS','LNP','KTB','MFL',"HBS", 'KIANI'
, 'SAK', 'TAB', 'ITCIKU', 'MSB', 'AEM', 'KTBB', 'KKEM', 'MBJ', 'TJY', 'TPH', 'INH PL', 'GTL', 'HCB', 'BMU'];
...
const replant = [
  ...
  // {kategori: 'Register', replant:'OTHE'},
  {kategori: 'Register', replant:'PAFA'},{kategori: 'Register', replant:'PCAR'},{kategori: 'Register', replant:'PECA'},
  ...
  {kategori: 'Register', replant:'TGRA'},{kategori: 'Register', replant:'TUP'},
  // {kategori: 'Register', replant:'UNKN'},
  ...
  // {kategori: 'Split', replant:'OTHE'},
  {kategori: 'Split', replant:'PAFA'},{kategori: 'Split', replant:'PCAR'},{kategori: 'Split', replant:'PECA'},
  ...
  {kategori: 'Split', replant:'TGRA'},{kategori: 'Split', replant:'TUP'}
  // {kategori: 'Split', replant:'UNKN'}
];
...
export default {
  ...
  data() {
    ...
    const store = new CustomStore({
      key: 'id',
      load: function () {
        return CrudService.getAll('registersplitinput/index');
      },
      insert: (key, values) => {
        return CrudService.insertData('registersplitinput', key, values);
      },
      remove: (key) => {
        return CrudService.removeData('registersplitinput/delete', key);
      },
  ...
  methods: {
    ...
    // getFilteredJenisbelukar: (options) => {
    //   return {
    //     store: jenisbelukar,
    //     filter: options.data ? ['kategori', '=', options.data.Kategori] : null,
    //   };
  ...
  created() {
    CrudService.findData('accessroles/check', { module: 'Registersplitinput' }).then(
      (response) => {
